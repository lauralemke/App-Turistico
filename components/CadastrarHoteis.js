...
export default class CadastrarHoteis extends Component {
  constructor(props) {
    super(props);
    this.state = {
      id: null,
...
this.props.navigation.goBack();
  ...
  carregarDados = async () => {
    ...
    const { hoteisKey } = route.params;

    const refHoteis = firebase.database().ref('hoteis/' + hoteisKey);

    var that = this;

    await refHoteis.once('value').then((snapshot) => {
      that.setState({
        id: hoteisKey,
  ...
  remover = (key) => {
    const vetorHoteis = this.state.arrayHoteis;

    vetorHoteis.splice(key, 1);
